...
import {
  createIntcodeMachine,
  createParallelIntcodeMachine
} from "./IntcodeMachineFactory";
...
export const useMachine = (id: string | undefined) => {
  const [machineState, setMachineState] = useState<DebugStateUpdate | null>(
    null
  );
...
export const useDay7Machines = (input: Array<number> = [5, 6, 7, 8, 9]) => {
  ...
  useEffect(() => {
    ...
    raw[0][2].setInitialValues([input[0], 0]);
    ...
    debugger;
    raw[0][2].setSourceRunner.call(raw[0][2], raw[4][0]);
    raw[1][2].setSourceRunner.call(raw[1][2], raw[0][0]);
    raw[2][2].setSourceRunner.call(raw[2][2], raw[1][0]);
    raw[3][2].setSourceRunner.call(raw[3][2], raw[2][0]);
    raw[4][2].setSourceRunner.call(raw[4][2], raw[3][0]);
